...
#include "stm32h5xx_hal.h" /* Provide the low-level HAL functions */
...
//(Note that the _256 is used as a mask to clear the prescaler bits as it provides binary 111 in the correct position)
...
#define CS_HIGH()	{HAL_GPIO_WritePin(SPI1_SD_CS_GPIO_Port, SPI1_SD_CS_Pin, GPIO_PIN_SET);}
#define CS_LOW()	{HAL_GPIO_WritePin(SPI1_SD_CS_GPIO_Port, SPI1_SD_CS_Pin, GPIO_PIN_RESET);}
...
{
#if USE_DMA
	HAL_SPI_Transmit_DMA(&SD_SPI_HANDLE, buff, (uint16_t) btx);
	while(HAL_SPI_GetState(&SD_SPI_HANDLE) != HAL_SPI_STATE_READY)
		;  // the bloody thing needs its time...
#endif

#if USE_IT
	HAL_SPI_Transmit_IT(&SD_SPI_HANDLE, buff, btx, HAL_MAX_DELAY);
#endif
