...
#if STOPMODE // MCU power save mode
...
misko_t* misko_ctor(void)
{
	...
	__misko.public.FlagPrint = 0;  // flag if in main() a debug print shall happen

	__misko.public.adxl345 = adxl345_ctor();  // initialize accelerometer
	__misko.public.fm25w256 = fm25w256_ctor();  // initialize RAM storage
	__misko.public.org1510 = org1510_ctor();  // initialize GPS receiver
