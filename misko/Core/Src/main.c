...
int main(void)
{
	...
	HAL_UART_Transmit(&huart2, (uint8_t*) uart2string, sizeof(uart2string), 100);		// transmit
...
{
	if(htim->Instance == TIM2)  // timer2 - 500ms periodic
		{
			HAL_GPIO_TogglePin(GPIOA, LED107_Pin);
		}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)		// ISR for pin change interrupts
{
	if(GPIO_Pin == PushButton_Pin)		// if button is pressed
		HAL_GPIO_TogglePin(GPIOB, LED101_Pin);		// for now only toggle a LED
}
