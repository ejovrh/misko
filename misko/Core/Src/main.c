...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
char bufferITOA1[sizeof(char) + 3];
char bufferITOA2[sizeof(char) + 3];
char bufferITOA3[sizeof(char) + 3];
char bufferITOA4[sizeof(char) + 3];
uint8_t len;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	/* bug in MX code generation - order or initialisation is wrong (UART before DMA -> no comm.)
	 ...
	 MX_DMA_Init();  // mem to mem: DMA2 C1 & C2
	 MX_GPIO_Init();
			...
			if(misko->FlagPrint)  // if printout is flagged
				{
					...
					itoa(misko->free1, (char*) bufferITOA1, 10);
					itoa(misko->free2, (char*) bufferITOA2, 10);
					itoa(misko->free3, (char*) bufferITOA3, 10);
//
					while(HAL_UART_GetState(&huart2) != HAL_UART_STATE_READY)
						;
					HAL_UART_Transmit_DMA(&huart2, (uint8_t*) bufferITOA1, strlen(bufferITOA1));
					...
					while(HAL_UART_GetState(&huart2) != HAL_UART_STATE_READY)
						;
					HAL_UART_Transmit_DMA(&huart2, (uint8_t*) bufferITOA2, strlen(bufferITOA2));
					...
					while(HAL_UART_GetState(&huart2) != HAL_UART_STATE_READY)
						;
					HAL_UART_Transmit_DMA(&huart2, (uint8_t*) bufferITOA3, strlen(bufferITOA3));

					while(HAL_UART_GetState(&huart2) != HAL_UART_STATE_READY)
						;
					HAL_UART_Transmit_DMA(&huart2, (uint8_t*) " - ", 3);

					itoa(strlen(misko->org1510->NMEA), (char*) bufferITOA4, 10);
					while(HAL_UART_GetState(&huart2) != HAL_UART_STATE_READY)
						;
					HAL_UART_Transmit_DMA(&huart2, (uint8_t*) bufferITOA4, strlen(bufferITOA4));

					//					HAL_UART_Transmit_DMA(&huart2, misko->gps->NMEA, strlen((const char*) misko->gps->NMEA));
//					len = strlen((const char*) misko->gps->NMEA);
//					HAL_UART_Transmit_DMA(&huart2, misko->gps->NMEA, len);
					while(HAL_UART_GetState(&huart2) != HAL_UART_STATE_READY)
						;
					HAL_UART_Transmit_DMA(&huart2, (uint8_t*) "\r\n", 2);
...
//			HAL_GPIO_TogglePin(LED_GREEN_GPIO_Port, LED_GREEN_Pin);
	...
	if(htim->Instance == TIM11)  // timer11 - 10ms periodic
		{
//			HAL_GPIO_TogglePin(LED2_GPIO_Port, LED2_Pin);  // for now just toggle the LED
			;
		}
}
