...
void EXTI4_IRQHandler(void)
{
  ...
  HAL_GPIO_EXTI_IRQHandler(EXTI_ADXL345_Pin);
...
void USART1_IRQHandler(void)
{
  /* USER CODE BEGIN USART1_IRQn 0 */
	...
	if(__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET)  // check if we have an idle interrupt
		{
			__HAL_UART_CLEAR_IDLEFLAG(&huart1);  // if so, clear it and indicate
...
void USART2_IRQHandler(void)
{
  /* USER CODE BEGIN USART2_IRQn 0 */
	if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_IDLE) != RESET)  // check if we have an idle interrupt
		{
			__HAL_UART_CLEAR_IDLEFLAG(&huart2);  // if so, clear it and indicate
		}
...
void EXTI15_10_IRQHandler(void)
{
  ...
  HAL_GPIO_EXTI_IRQHandler(EXTI_PushButton_Pin);
...
void DMA2_Channel2_IRQHandler(void)
{
  ...
  HAL_DMA_IRQHandler(&hdma_memtomem_dma2_channel2);
