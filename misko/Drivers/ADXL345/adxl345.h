...
#define DEVICE_ID   				0x00																								// Device ID, DS. p. 24
...
{
	volatile uint8_t FlagStop;  // flag for processor stop mode

	uint8_t (*ReadByte)(uint8_t in_addr);  // reads one byte of data from address
	void (*WriteByte)(uint8_t in_addr, uint8_t in_data);	// writes one byte of data to address
	void (*ISR)(void);  // ISR for device
} adxl345_t;

adxl345_t* adxl345_ctor(void);  // declare constructor for concrete class

extern adxl345_t *adxl345;  // declare pointer to public struct part
