 ...
 static uint8_t _ReadByte(const uint8_t in_addr)								// reads one byte of data from address
{
	...
	retval = __ADXL345._spi->SPIReadByte(in_addr | 0x80);						// request data from address
...
{
	gpio_clr(SPI_SS_ADXL345_pin);												// drive device CS low
	__ADXL345._spi->SPIWriteByte(in_addr, in_data);								// write data to address
	gpio_set(SPI_SS_ADXL345_pin);												// drive device CS high
};
...
adxl345_t *adxl345_ctor(void)													// object constructor
{
	...
	// accelerometer device initialization & configuration
	{
		...
		_WriteByte(THRESH_ACT, 0x5);											// 62.5mg/LSB
		_WriteByte(THRESH_INACT, 0x5);											// 62.5mg/LSB
...
ISR(INT7_vect)
{
	...
	__ADXL345.adxl345_irq_src = __ADXL345._spi->SPIReadByte(INT_SOURCE);		// clear pending interrupts

	// inactivity
	if( (__ADXL345.adxl345_irq_src >> 3) & 0x01)								// if the inact. bit is set
	{
		gpio_set(gps_red_led_pin);												// indicate
	}

	// activity
	if( (__ADXL345.adxl345_irq_src >> 4) & 0x01)								// if the act bit is set
	{
		gpio_clr(gps_red_led_pin);												// indicate
	}
