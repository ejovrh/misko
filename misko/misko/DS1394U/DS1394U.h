...
typedef struct																	// public part of struct describing devices on DS1394
{
	uint8_t (*Read)(const uint8_t in_addr);										// reads one byte from address
	uint8_t (*Write)(uint8_t in_addr, const uint8_t in_val);					// writes a byte of value to address
	void (*SetDate)(const unsigned char in_day, const unsigned char in_day_date, const unsigned char in_month, const unsigned char in_year);//
	void (*SetTime)(const unsigned char in_hours, const unsigned char in_minutes, const unsigned char in_seconds);//
